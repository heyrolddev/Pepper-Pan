...
import { canonical, SHOP, siteUrl } from "@/lib/site";
...
async function getMenu(): Promise<{
  ...
  try {
    ...
    /**
     * Everything at once, because none of it depends on anything else.
     *
     * These six reads used to be six `await`s in a row: dishes, then ratings,
     * then the whole stock picture (four more tables), then categories, then
     * the menu cards, then the add-ons. Nothing in that list needed the
     * answer to the one before it, so the page sat waiting out six round
     * trips to Supabase end to end before it could render a byte — which on a
     * phone on mobile data is most of a second of blank screen, every single
     * time somebody opens the menu.
     *
     * The one real dependency — the add-ons want the stock picture to know
     * which are sold out — is handed over un-awaited, so those queries go out
    ...
    const [
      ...
      supabase.from("meal_ratings").select("meal_id, avg_rating, review_count"),

      // The shop's categories and their colours. A failure here is not a
      // reason to fail the menu: `colourOf` works out a stable colour from
      // the name when there's no row, so the worst case is the owner's chosen
      // colours being replaced by sensible ones rather than a page that won't
      // load.
      supabase
      ...
      /**
       * The groupings. A failure here still does not fail the menu — every
       * dish renders as its own card, which is what the menu was before
       * anybody grouped anything — but it is no longer SILENT.
       *
       ...
       * should show one card called whatever the owner named it. On screen
       * that is indistinguishable from "renaming the card broke it", which is
       * how it was reported — and there was nothing in any log to say
       * otherwise.
       */
      supabase
        .from("menu_products")
      ...
      /**
       * The add-ons. An option's "sold out" is its DISH's sold out — "Extra
       * rice" greys when the rice runs out — so this wants the stock picture,
       * and gets it as a promise rather than a value.
       *
       ...
       */
      loadModifiers(supabase, stock),

      // How many the shelf can still make. Derived, never written back to
      // `is_available` — that switch is the owner's own "we've 86'd it
